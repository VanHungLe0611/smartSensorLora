...
sys.path.insert(0, 'board_config')
from board_config import boards_properties
...
###############################################################
vars = Variables()
vars.Add('board', 'choose supported board(see more in board_config)', 0)
env = Environment(variable = vars, ENV = os.environ, tools = ['mingw'], exports='env')
...
# init boards configuration parameter
board_option = ARGUMENTS.get('board', 0)
if board_option==0:
    board = boards_properties['stm32l496RGTx']
    #board = boards_properties['stm32f767ZITx']
    #board = boards_properties['stm32h743IITx']
else:
    board = boards_properties[board_option]
print('+++++++++++++++++++++++++ board =' + str(board_option))
linker_dir = './board_config/linker/'
startup_dir = './board_config/startup/'
Export('board')

include_dirs = [
     # Third party
    '#Drivers/CMSIS/Include/',
    '#Drivers/'+board['CMSIS_DIR']+'/Include/',
    '#Drivers/'+board['STM_HAL_DIR']+'/Inc/',
    '#Drivers/'+board['STM_HAL_DIR']+'/Inc/Legacy/',
    #'#Middlewares/ST/STM32_USB_Device_Library/Class/CDC/Inc',
    #'#Middlewares/ST/STM32_USB_Device_Library/Core/Inc',
    #'#Middlewares/Third_Party/FreeRTOS/Source/include/',
    #'#Middlewares/Third_Party/FreeRTOS/Source/CMSIS_RTOS/',
    #'#Middlewares/Third_Party/FreeRTOS/Source/portable/GCC/'+board['FreeRTOS_DIR'],
    ...
    #'#lib/driver/sdram/',
    #'#lib/driver/gsm/',
    #'#lib/driver/gsm/uart_gsm/',
    #'#lib/driver/error_handler/',
    #'#lib/driver/i_gpio',
    #'#util/',
    #'#lib/driver/',
     # local
...
ccflags = [
    ...
    '-fmessage-length=0'       , '-mfpu='+board['FPU'], '-mfloat-abi=softfp' , '-w'                   ,
    '-Wextra'                ]
...
# linker flags
linkflags = [
    '-mcpu='+board['CPU']     , '-mthumb'         , '-T'+linker_dir+board['LINKER'] , '-fsigned-char'        ,
    '-ffunction-sections' , '-fdata-sections' , '-fmessage-length=0'       , '-mfpu='+board['FPU']    ,
...
asppflags = [ '-mcpu='+board['CPU']]
#User-specified C preprocessor options
cppdefines = [ 'USE_HAL_DRIVER', board['MCU'], 'HSE_VALUE=8000000',  'USE_FULL_LL_DRIVER']
...
for folder in list(set(src_folders)): # avoid duplicates
    ...
    if not os.path.exists(sconscript_name):
        ...
        objs.append(o)
objs.append(env.Object(startup_dir + board['STARTUP']))
###### end add src folders ######
