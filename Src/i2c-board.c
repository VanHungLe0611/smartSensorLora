...
void I2cMcuInit(I2c_t *obj, I2cId_t i2cId, PinNames scl, PinNames sda) {
	...
	I2cHandle.Instance = (I2C_TypeDef*) I2C3_BASE;

	GpioInit(&obj->Scl, scl, PIN_ALTERNATE_FCT, PIN_OPEN_DRAIN, PIN_NO_PULL,
			GPIO_AF4_I2C3);
	GpioInit(&obj->Sda, sda, PIN_ALTERNATE_FCT, PIN_OPEN_DRAIN, PIN_NO_PULL,
			GPIO_AF4_I2C3);
}
...
void I2cMcuResetBus(I2c_t *obj) {
	...
	GpioInit(&obj->Scl, I2C_SCL, PIN_ALTERNATE_FCT, PIN_PUSH_PULL, PIN_NO_PULL,
			GPIO_AF4_I2C3);
	GpioInit(&obj->Sda, I2C_SDA, PIN_ALTERNATE_FCT, PIN_PUSH_PULL, PIN_NO_PULL,
			GPIO_AF4_I2C3);

	I2cMcuFormat(obj, MODE_I2C, I2C_DUTY_CYCLE_2, true, I2C_ACK_ADD_7_BIT,
			100000);
}
...
void I2cMcuDeInit(I2c_t *obj) {
	...
	GpioInit(&obj->Scl, obj->Scl.pin, PIN_ANALOGIC, PIN_PUSH_PULL, PIN_NO_PULL,
			0);
	GpioInit(&obj->Sda, obj->Sda.pin, PIN_ANALOGIC, PIN_PUSH_PULL, PIN_NO_PULL,
			0);
}
...
uint8_t I2cMcuWriteBuffer(I2c_t *obj, uint8_t deviceAddr, uint16_t addr,
	...
	status =
			(HAL_I2C_Master_Transmit(&I2cHandle, deviceAddr, buffer, size, 2000)
					== HAL_OK) ? SUCCESS : FAIL;
...
uint8_t I2cMcuReadBuffer(I2c_t *obj, uint8_t deviceAddr, uint16_t addr,
	...
	status =
			(HAL_I2C_Master_Receive(&I2cHandle, deviceAddr, buffer, size, 2000)
					== HAL_OK) ? SUCCESS : FAIL;
...
uint8_t I2cMcuWaitStandbyState(I2c_t *obj, uint8_t deviceAddr) {
	...
	status =
			(HAL_I2C_IsDeviceReady(&I2cHandle, deviceAddr, 300, 4096) == HAL_OK) ?
					SUCCESS : FAIL;
	;
